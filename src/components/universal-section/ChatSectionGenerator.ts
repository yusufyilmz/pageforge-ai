...
class ChatSectionGenerator {
  ...
  detectSectionRequest(message: string): boolean {
    ...
    const hasSection = sectionKeywords.some((keyword) => message.toLowerCase().includes(keyword));

    const hasContent = contentKeywords.some((keyword) => message.toLowerCase().includes(keyword));
  ...
  extractSectionRequirements(message: string): {
    ...
    // Contact section
    else if (messageLower.includes("contact") || messageLower.includes("form")) {
      type = "contact-form";
      name = "ContactForm";
      suggestedContent = {
        title: "Get In Touch",
    ...
    else if (messageLower.includes("review") || messageLower.includes("testimonial")) {
    ...
    // Services section
    else if (messageLower.includes("service") || messageLower.includes("offering")) {
      type = "services-grid";
      name = "ServicesGrid";
      suggestedContent = {
        title: "Our Services",
  ...
  async generateSectionFromChat(
    ...
    try {
      ...
      return {
        success: false,
        message: "Sorry, there was an error generating your section. Please try again.",
      };
    }
  }
