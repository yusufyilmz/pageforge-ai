...
export const AISectionBuilder = ({ onSectionsGenerated, onClose }: AISectionBuilderProps) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const intents = await aiSectionGenerator.analyzeAndGenerateSections(requirement);
  ...
  const handleGenerateSections = async () => {
    ...
    try {
      ...
      for (const intent of generatedIntents) {
        const spec = await aiSectionGenerator.generateSectionSpecification(intent, requirement);
    ...
    return (
      <Flex direction="column" gap="l" fillWidth>
        ...
        <Text variant="body-default-m">
          Describe what you want on your webpage, and I&apos;ll automatically generate the
  ...
  if (step === "review") {
    return (
      <Flex direction="column" gap="l" fillWidth>
        ...
        <Text variant="body-default-m">
          I&apos;ve analyzed your requirements and suggest these sections. You can remove or reorder
            ...
            >
              <Flex direction="column" fillWidth gap="xs">
                <Heading variant="heading-strong-s">
                  {intent.type.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())}
                </Heading>
              ...
              <Flex gap="s" align="center">
                <input
                  ...
                  onChange={(e) => updateIntentPriority(index, Number.parseInt(e.target.value))}
                  style={{
                    width: "60px",
                    padding: "4px",
                    border: "1px solid #ccc",
        ...
        <Flex gap="m">
          ...
          >
            {isGenerating ? "Generating..." : `Generate ${generatedIntents.length} Sections`}
          </Button>
  ...
  return (
    <Flex direction="column" gap="l" fillWidth align="center">
      ...
      <Text variant="body-default-m" style={{ textAlign: "center" }}>
        Your custom sections have been generated and are ready to use. They&apos;ve been
...
export const useAISectionGeneration = () => {
  ...
  const generateSectionsFromChat = async (
    ...
    try {
      ...
      const industry = context?.industry || extractIndustryFromMessage(chatMessage);
      const pageType = context?.pageType || extractPageTypeFromMessage(chatMessage);
