...
export class AutoSectionRegistry {
  ...
  async loadSection(type: string): Promise<SectionComponent | null> {
    ...
    console.warn(`Section type '${type}' not found in predefined or AI-generated sections`);
    return null;
  }

  private async loadPredefinedSection(type: string): Promise<SectionComponent | null> {
    try {
  ...
  async generateSectionFromAI(
    ...
    try {
      ...
      console.log(`✅ Generated AI section: ${config.name} (type: ${config.type})`);
      return config.type;
    } catch (error) {
