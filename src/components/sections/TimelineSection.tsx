...
import type { ContentBlock, TimelineSectionContent } from "@pageforge/types/page/pageTypes";
...
export const TimelineSection = ({ block, index }: TimelineSectionProps) => {
  ...
  const renderTimelineItem = (item: TimelineSectionContent["items"][0], itemIndex: number) => (
    <Flex key={itemIndex} gap="l" vertical="start">
      ...
      <Column horizontal="center" gap="xs" minWidth={60}>
        <Column
          ...
          padding="s"
        >
          <Icon name={item.icon || "circle"} size="s" onBackground="neutral-strong" />
        </Column>
        {showDates && (
