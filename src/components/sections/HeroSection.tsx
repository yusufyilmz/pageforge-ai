...
import { Badge, Button, Column, Flex, Heading, Icon, Text } from "@pageforge/once-ui/components";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Description */}
        {description && (
          <Column horizontal={alignmentMap[alignment]} maxWidth="m" marginBottom="m">
            <Text
          ...
          <Flex gap="m" wrap horizontal={alignmentMap[alignment]} fillWidth>
            {buttons.map((button, btnIndex: number) => (
              <Button
                key={btnIndex}
                href={button.href}
                variant={button.variant || (btnIndex === 0 ? "primary" : "secondary")}
                size={button.size || "l"}
                data-border="rounded"
          ...
          <Flex gap="m" marginTop="l" wrap horizontal={alignmentMap[alignment]}>
            {content.features.map((feature, featureIndex: number) => (
              <Flex key={featureIndex} gap="xs" vertical="center">
                <Icon name={feature.icon || "check"} onBackground="accent-weak" size="s" />
