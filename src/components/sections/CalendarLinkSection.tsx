...
export const CalendarLinkSection = ({ block, index }: CalendarLinkSectionProps) => {
  ...
  return (
    ...
    >
      ...
      <Flex paddingX="8" direction="column">
        <span>{text}</span>
        {description && <span style={{ fontSize: "0.875rem", opacity: 0.8 }}>{description}</span>}
      </Flex>
      <IconButton href={link} data-border="rounded" variant="secondary" icon="chevronRight" />
    </Flex>
  );
};
