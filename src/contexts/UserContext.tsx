...
interface UserContextType {
  ...
  updateWorkExperience: (
    id: string,
    experience: Partial<UserWorkExperience>
  ) => void
...
export function UserProvider({ children, initialData }: UserProviderProps) {
  ...
  const updateWorkExperience = (
    id: string,
    experienceUpdate: Partial<UserWorkExperience>
  ) => {
    setUserInfo(prev => ({
      ...prev,
  ...
  const updateEducation = (
    id: string,
    educationUpdate: Partial<UserEducation>
  ) => {
    setUserInfo(prev => ({
      ...prev,
    ...
    setUserInfo(prev => {
      ...
      return {
        ...
        stats: {
          ...prev.stats,
          projectsCompleted:
            prev.stats.projectsCompleted - (wasCompleted ? 1 : 0)
        },
        lastUpdated: new Date().toISOString()
      }
    })
...
export function useUserSocialLinks(onlyVisible = true) {
  const { userInfo } = useUser()
  return onlyVisible
    ? userInfo.socialLinks.filter(link => link.display)
